...
// When a promise rejects, the control jumps to the 
// closest rejection handler. 
// That’s very convenient in practice.

// For instance, in the code below the URL to fetch is wrong 
// (no such site) and .catch handles the error:

fetch('https://no-such-server.blabla') // rejects
  .then(response => response.json())
  .catch(err => alert(err)) // TypeError: failed to fetch (the text may vary)

// As you can see, the .catch doesn’t have to be immediate. 
// It may appear after one or maybe several .then.

// Or, maybe, everything is all right with the site, 
...
// But if any of the promises above rejects 
// (a network problem or invalid json or whatever), then it would catch it.

// Implicit try…catch

// The code of a promise executor and promise handlers 
// has an "invisible try..catch" around it. 
...
// The "invisible try..catch" around the executor automatically 
// catches the error and turns it into rejected promise.

// This happens not only in the executor function, 
// but in its handlers as well. 
...
// As we already noticed, .catch at the end of the chain is similar to try..catch. 
// We may have as many .then handlers as we want, 
// and then use a single .catch at the end to handle errors in all of them.

// In a regular try..catch we can analyze the error and maybe rethrow 
// it if it can’t be handled. The same thing is possible for promises.

// If we throw inside .catch, then the control goes to the next 
// closest error handler. And if we handle the error and finish normally, then it continues to the next closest successful .then handler.
...
// Here the .catch block finishes normally. 
// So the next successful .then handler is called.

// In the example below we see the other situation with .catch. 
// The handler (*) catches the error and just can’t handle it 
// (e.g. it only knows how to handle URIError), so it throws it again:
...
// But there is none. So the error gets “stuck”. There’s no code to handle it.
...
// in that case. You can see it in the console if you run the example above.
...
// If an error occurs, and there’s no .catch, the unhandledrejection 
// handler triggers, and gets the event object with the 
// information about the error, so we can do something.
...
// In non-browser environments like Node.js there are other ways 
// to track unhandled errors.

// Summary

// .catch handles errors in promises of all kinds: be it a reject() call, 
// or an error thrown in a handler.
// We should place .catch exactly in places where we want to 
// handle errors and know how to handle them. 
