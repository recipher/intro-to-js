...
// until the promise settles, and then resumes it with the promise result. 
// That doesn’t cost any CPU resources, because the JavaScript engine 
// can do other jobs in the meantime: 
// execute other scripts, handle events, etc.

// It’s just a more elegant syntax of getting the promise result 
// than promise.then. And, it’s easier to read and write.

// Can’t use await in regular functions

// If we try to use await in a non-async function, 
...
// Let’s take the showAvatar() example from the section 
...
// People who are just starting to use await tend to forget the fact 
// that we can’t use await in top-level code. For example, this will not work:
...
// The meaning is the same: it ensures that the returned value 
// is a promise and enables await.
...
// If we don’t have try..catch, then the promise generated 
// by the call of the async function f() becomes rejected. 
...
// as described in the chapter Error handling with promises.
...
// we’re syntactically unable to use await, so it’s a normal practice 
// to add .then/catch to handle the final result or 
...
// and then becomes an exception that we can catch using 
...
// // Together they provide a great framework to write 
// asynchronous code that is easy to both read and write.
