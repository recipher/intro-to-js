...
// We’ll quickly cover their use cases here.
...
// For instance, download several URLs in parallel and process the content 
...
// (it technically can be any iterable, but is usually an array) 
...
// Please note that the order of the resulting array members 
// is the same as in its source promises. 
...
// A bigger example with fetching user information for an array 
// of GitHub users by their names 
// (we could fetch an array of goods by their ids, the logic is identical):
...
// If any of the promises is rejected, the promise returned 
// by Promise.all immediately rejects with that error.
...
// completely forgetting about the other ones in the list. 
// Their results are ignored.

// For example, if there are multiple fetch calls, 
// like in the example above, and one fails, 
// the others will still continue to execute, 
// but Promise.all won’t watch them anymore. 
...
// Normally, Promise.all(...) accepts an iterable (in most cases an array) 
// of promises. 
// But if any of those objects is not a promise, 
...
// That’s good for “all or nothing” cases, 
...
// Similar to Promise.all, but waits only for the first 
// settled promise and gets its result (or error).
...
// Similar to Promise.race, but waits only for the 
// first fulfilled promise and gets its result. 
// If all of the given promises are rejected, 
// then the returned promise is rejected with 
// AggregateError – a special error object that stores all promise errors 
...
// so the second promise became the result. 
...
// As you can see, error objects for failed promises 
// are available in the errors property of the AggregateError object.

Promise.resolve/reject

// Methods Promise.resolve and Promise.reject are rarely needed in modern code, 
// because async/await syntax (we’ll cover it a bit later) 
// makes them somewhat obsolete.

// We cover them here for completeness and for those who 
// can’t use async/await for some reason.
...
// For example, the loadCached function below fetches a URL 
// and remembers (caches) its content. 
// For future calls with the same URL it immediately gets 
// the previous content from cache, 
// but uses Promise.resolve to make a promise of it, 
...
// We can write loadCached(url).then(…), because the function 
// is guaranteed to return a promise. 
// We can always use .then after loadCached. 
// That’s the purpose of Promise.resolve in the line (*).
...
// Promise.all(promises) – waits for all promises to resolve and 
